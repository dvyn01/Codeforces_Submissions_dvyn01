...
public:
  ...
  inline int readString(char *arg, int limit, const char *terminators) {
    if (!*terminators)
      return readString(arg, limit, InputDevice::isSpace);
  ...
  template <class F>
  inline typename enable_if<is_floating_point<F>::value, bool>::type
  read(F &arg) {
  ...
  inline typename enable_if<is_integral<I>::value, bool>::type
  ...
  template <class T1, class T2> inline bool read(pair<T1, T2> &arg) {
  ...
  inline typename enable_if<is_iterator<Iterator>::value, bool>::type
  ...
  // generic forwarding
  template <class T> inline auto read(T &arg) -> decltype(arg.read(*this)) {
...
public:
  InputFile(FILE *file = stdin, bool lineBuffered = true,
      ...
      : InputDevice(buffer, buffer), file(file), lineBuffered(lineBuffered),
        owner(takeOwnership) {}
...
protected:
  ...
  OutputDevice()
      : output(buffer), end(buffer + BUFFER_SIZE + MAX_LENGTH), separate(false),
        width(setWidth().value), fill(setFill().value), base(setBase().value),
        precision(setPrecision().value), delimiter(setDelimiter().value) {
    computeBasePower();
  }
...
public:
  ...
  template <class T> inline int write(const complex<T> &arg) {
    return write(real(arg), imag(arg));
  }
  // ranges
  template <class Iterator, class... Ts>
  inline typename enable_if<is_iterator<Iterator>::value, int>::type
  ...
  template <class T0, class T1, class... Ts>
  inline typename enable_if<!is_iterator<T0>::value, int>::type
...
template <class... Ts> inline bool read(Ts &&... args) {
  return input->read(forward<Ts>(args)...);
}
template <class... Ts> inline int write(Ts &&... args) {
  return output->write(forward<Ts>(args)...);
}
template <class... Ts> inline int writeln(Ts &&... args) {
 ...
 * Strings: read until character limit is reached or termination character is
 *found (one of the characters in a given string or defined by predicate,
 *isspace by default)
 * - char (&)[N], terminator
 ...
 * - setWidth(uint): minimum width of a single element to write (except
 *character)
 ...
 * - setDelimiter(const char*): delimiter automatically inserted between
 *elements that are not strings or characters Single variable of one of the
 *following types:
 * - char, standard integer and floating point types
 * - string, const char*
 * - pair, complex
...
struct Data {
  ll sx, sy, x, y, d;
  Data(ll sx, ll sy, ll x, ll y, ll d) {
    this->sx = sx, this->sy = sy, this->x = x, this->y = y, this->d = d;
  }
};

bool safe(ll x, ll y) { return x >= 0 && x < n && y >= 0 && y < m; }

pair<ll, ll> getNext(ll i, ll j) {
  if (s[i][j] == '>')
    return {i, j + 1};
  if (s[i][j] == '<')
    return {i, j - 1};
...
bool dfs(ll i, ll j) {
  if (s[i][j] == '#')
    return false;
  vis[i][j] = 1;
  pair<ll, ll> nx = getNext(i, j);
  ll x = nx.first, y = nx.second;
  bool k = false;
  if (vis[x][y] == 0)
    k |= dfs(x, y);
  else if (vis[x][y] == 1)
...
int main() {
  ...
  queue<Data> q;
  map<pair<ll, ll>, vll> start;
  f(0, n) {
    fi(0, m) {
      if (s[i][j] == '#') {
        if (safe(i - 1, j) && s[i - 1][j] == 'v')
          q.push(Data(i - 1, j, i - 1, j, 1));
        if (safe(i + 1, j) && s[i + 1][j] == '^')
          q.push(Data(i + 1, j, i + 1, j, 1));
        if (safe(i, j - 1) && s[i][j - 1] == '>')
          q.push(Data(i, j - 1, i, j - 1, 1));
        if (safe(i, j + 1) && s[i][j + 1] == '<')
          q.push(Data(i, j + 1, i, j + 1, 1));
      }
    }
  ...
  while (!q.empty()) {
    Data c = q.front();
    q.pop();
    if (start.find({c.sx, c.sy}) == start.end())
      start[{c.sx, c.sy}];
    if (start[{c.sx, c.sy}].size() == 2)
      start[{c.sx, c.sy}][0] = start[{c.sx, c.sy}][1],
                   start[{c.sx, c.sy}][1] = c.d;
    else
      start[{c.sx, c.sy}].pb(c.d);
    if (safe(c.x - 1, c.y) && s[c.x - 1][c.y] == 'v')
      q.push(Data(c.sx, c.sy, c.x - 1, c.y, c.d + 1));
    if (safe(c.x + 1, c.y) && s[c.x + 1][c.y] == '^')
      q.push(Data(c.sx, c.sy, c.x + 1, c.y, c.d + 1));
    if (safe(c.x, c.y - 1) && s[c.x][c.y - 1] == '>')
      q.push(Data(c.sx, c.sy, c.x, c.y - 1, c.d + 1));
    if (safe(c.x, c.y + 1) && s[c.x][c.y + 1] == '<')
      q.push(Data(c.sx, c.sy, c.x, c.y + 1, c.d + 1));
  }
  ll ans = 0;
  ...
  for (auto it : start) {
    if (it.second.size() > 1) {
      if (it.second[it.second.size() - 1] != it.second[it.second.size() - 2])
        ans = max(ans, it.second[it.second.size() - 1] +
                           it.second[it.second.size() - 2]);
      else
        ans = max(ans, it.second[it.second.size() - 1] +
                           it.second[it.second.size() - 2] - 1);
    }
    if (v.size() == 0) {
      v.pb(it.second.back());
    } else if (v.size() == 1) {
      ...
      if (it.second.back() > v[1])
        v[0] = v[1], v[1] = it.second.back();
      else if (it.second.back() > v[0])
        v[0] = it.second.back();
    }
  }
  if (v.size() > 1)
